...
const string workload = "../workloads/"; // done: the workload folder filepath

const string load = workload + "1w-rw-50-50-load.txt"; // done: the workload_load filename
const string run  = workload + "1w-rw-50-50-run.txt"; // done: the workload_run filename

const int READ_WRITE_NUM = 10000; // done: amount of operations
...
{        
    ...
    while(fgets(buf_fptree,32,ycsb_load) != NULL){
    ...
    // done: run the workload_run in levelDB
    // DONE
     for (i = 0; i < operation_num; i++) {
    	//增其实就是改 因为key == value 
        if (op[i] == 0 && op[i] == 2) {
