...
(写入后不调用，发生系统崩溃有什么后果)  
clflush的作用是从缓存中取出缓存线并将其内容写回内存，写入后不调用，如果发生系统崩溃，会如同上面的文件IO以及文件映射那样，修改的数据会发生丢失，即我们修改的数据并没有真正写回到内存中（SCM）。 调用CLFLUSH等指令是为了保证SCM写的顺序和持久性。
...
数据库将内存数据（各类表）以文件的形式保存在永久介质中(磁盘等)，持续化指针用来指向这些数据，而且持久化指针存储在SCM中能能持久储存。所以可以利用持久化指针在重新启动地址空间重新分配的情况下，恢复储存在SCM的数据，同时可以通过再分配时不断刷新持久化指针达到避免内存泄露的作用。在我认为比较像课上学的RID比较像，持久化指针组成是8位文件ID与8位文件内偏移，与RID中的页号，槽号类似